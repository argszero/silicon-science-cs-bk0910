...
**Status**: revision v1.2 (2026-09-07) — responding to major-revision round 1 (freeze arms extended to 5 seeds/arm with disjoint CIs; single-environment excursion example marked at every use; monitoring claim scoped to the tested SGD recipe; λmax sampling-bias direction stated).
...
We find that no single 2026 mechanism accounts for the phase map. (i) Weight decay collapses scale-invariant norms in every wd>0 run (9.2→0.14 at wd=0.03) yet most runs never spike: 30 runs with ≥50% norm collapse had zero spikes (collapse→spike accord 18/48 = 37.5%, CI[25.2,51.6]). (ii) λmax crossing 2/η is equally insufficient: 15 clean runs crossed the threshold with no spikes, and the causal freeze arms (r2, 5 seeds/arm, post-branch outcomes) show the load-bearing condition is the hidden-projection adaptation: freezing fc1/fc2 gives **0/5 spiked** (CI[0,43.4]) vs control 5/5 (CI[56.6,100]) vs freeze-readout 5/5 (CI[56.6,100]) — disjoint CIs — while committed-reference traces show sharpness *excursions* to 6.6× the threshold persisting in freeze=hid with zero spikes (single-environment realization; the environment-robust evidence is the r2 freeze contrast). (iii) Precision is not the driver: fp64 training spikes as much as fp32 (2/2 cells). The causal decomposition identifies the load-bearing condition: spikes require **both** a sharp regime (λmax reaching ≥2/η) **and** ongoing adaptation of the scale-invariant hidden projection weights — removing either suppresses spikes, and a trainable-subspace (restricted) λmax equals the full-Hessian λmax, ruling out an effective-sharpness explanation. Each 2026 single-mechanism account captures one necessary condition of this chain; none is sufficient. The falsifiable claim: in this toy regime, monitoring any single one of {norm, λmax, precision} is insufficient to predict spikes; a dynamical (co-adaptation) condition is required.
...
Our core finding, stated for the toy regime: **spikes require both a sharp regime and ongoing adaptation of the scale-invariant hidden projections; every 2026 single-mechanism account is necessary-but-not-sufficient at this scale, and sharpness excursions alone do not spike when the hidden projection weights are frozen.** The practical consequence is a monitoring warning, **scoped to the tested recipe — SGD-trained, LayerNorm-normalized MLPs with weight decay**: none of the three single-mechanism diagnostics (norm trajectory, λmax crossing, precision) is sufficient to anticipate spikes in this regime; a dynamical co-adaptation condition is required, pointing to the non-normal amplification frame as the unifying mechanism [4]. Transfer to the AdamW/transformer default recipe is not established by this study and is future work (as registered); the claims here are bounded to plain-SGD normalized networks.

## 2 Related work (with stated differences)

1. **Edge of Stability (Cohen et al. 2021)** [1] — sharpness λmax crossing 2/η as the instability threshold for gradient descent. *Difference*: we test sufficiency on a shared phase map and find 15 clean runs above threshold (54.5% accord CI[38.0,70.2]), and the causal freeze arms (freeze=hid 0/5 post-branch, CI[0,43.4], vs control 5/5, CI[56.6,100]) show sharpness alone is insufficient — a committed-reference trace even sustains excursions to 6.6× the threshold with zero spikes once the hidden projections are frozen — sharpness alone is insufficient in our controlled regime.
...
Across the 33 runs that ever crossed λmax = 2/η, only 18 spiked: accord 54.5% (Wilson CI[38.0,70.2]), statistically indistinguishable from a coin flip. The violations are not transient artifacts of our λmax sampling: cell means are stable (e.g. η=0.05 wd=0.1: post-collapse median λmax 37.5 < 2/η=40 yet seeds 1–2 spike 5/7 times; η=0.05 wd=0.1 seed 0 reaches λmax 121 — 3× threshold — and never spikes). Per-step sampling at onset (Appendix B) shows spikes co-occur with λmax *excursions* in control arms. Sampling-bias direction: phase-map λmax is measured every 400 steps, so a run classified "clean but crossed 2/η" could only be misclassified by having crossed *between* samples undetected; the true crossing count is therefore ≥ the observed 33, and the true accord is ≤ the observed 54.5% (extra crossings would add clean-crossed runs). The P2 refutation is conservative: denser sampling would only strengthen it. (The claim that crossing is not sufficient relies on the existence of clean runs above threshold at the sampled resolution — Appendix B's per-20-step onset runs confirm the excursion structure at the event timescale.)

### 4.4 Causal freeze decomposition (Figures 2–3)

On the money cell (η=0.2 wd=0.1, branch at step 300, **5 seeds per arm**), the outcome measure is **post-branch spike events** (start ≥ step 300): pre-branch events are shared bit-identically across arms (seeds 2–4 each show one early transient at steps 125–200 in *every* arm, control included) and are not attributable to the freeze. Post-branch: control spikes resume in **5/5 seeds** (CI[56.6,100]; 9–22 events); **freeze-hidden (fc1+fc2 frozen) → 0 post-branch spikes in all five seeds (0/5, CI[0,43.4])**; freeze-readout (fc3) → spikes persist in 5/5 (CI[56.6,100]; 4–9 events). The control vs freeze=hid CIs are **disjoint** ([56.6,100] vs [0,43.4]); test accuracy stays ≥0.90 in all arms. The hidden projection weights' *adaptation* — not their norm level, not the readout, not the λmax level — is the load-bearing condition for the instability.

![Figure 2: freeze-arm outcomes on the money cell (η=0.2, wd=0.1), r2.](figures/fig2_freeze_arms.png)

*Figure 2: causal freeze arms (r2, branch at step 300, 5 seeds/arm): fraction of seeds with ≥1 post-branch spike — control 5/5 (CI[56.6,100]), freeze=hid (fc1+fc2 frozen) 0/5 (CI[0,43.4]), freeze=readout (fc3 frozen) 5/5 (CI[56.6,100]) — disjoint CIs; per-seed post-branch full-Hessian λmax means overlaid where r3 measured them (seeds 0–1; freeze=hid s1 25.2 > 2/η = 10 while clean, committed-reference). The freeze=hid arms keep above-threshold sharpness yet do not spike; the readout is not the load-bearing group.*
...
freeze-arm direction (freeze=hid 0/5 vs control 5/5 vs freeze=readout 5/5 post-branch — at n=2 reproduced exactly in two environments, extended to n=5 in the committed reference), fp64 persistence, and the
...
**Prior-belief reconciliation.** Registered priors (issue #86): P1 (weight-norm collapse suffices) — **refuted** (accord 37.5%, CI upper bound 51.6); P2 (λmax>2/η suffices) — **refuted** (accord 54.5%, CI[38.0,70.2]; r2 freeze arms: freeze=hid 0/5 post-branch CI[0,43.4] vs control 5/5 CI[56.6,100]; committed-reference trace shows a 6.6×-threshold excursion with zero spikes in freeze=hid — single-environment); P3 (a second-order discriminator is required) — **confirmed and refined** to the two-condition co-adaptation rule. The refutations of the two theory-anchored sufficiency priors are the study's strong-novelty signal: the pilot data that motivated registration already showed the tension, and the full phase map + causal arms resolve it.

## 6 Conclusion

On a shared toy benchmark with concurrent measurement of every 2026 loss-spike mechanism's named trigger, no single mechanism suffices: weight decay's norm collapse is necessary-not-sufficient (37.5% accord), λmax crossing 2/η is necessary-not-sufficient (54.5% accord), fp precision is irrelevant (fp64 spikes), and freezing the hidden projection weights suppresses post-branch spikes in all five seeds (0/5, CI[0,43.4], vs control 5/5, CI[56.6,100] — disjoint CIs), even though committed-reference traces show sharpness excursions to 6.6× the threshold persisting while frozen (single-environment realization; the r2 freeze contrast is the environment-robust evidence). The toy regime (plain-SGD, LayerNorm-normalized MLPs with weight decay) is governed by a two-condition dynamical rule — sharp regime plus hidden-projection co-adaptation — consistent with non-normal transient amplification as the unifying mechanism. We contribute the benchmark, the concurrent-diagnostics protocol, the causal freeze-arm decomposition, and the falsification of three 2026 sufficiency claims; the instrument and data are committed for byte-identical reproduction.
...
- Figure 2 (fig2_freeze_arms.png): freeze-arm outcome bars — control 5/5 spiked post-branch vs freeze-hidden 0/5 vs freeze-readout 5/5 (n=5/arm, disjoint CIs), with per-seed post-branch λmax means overlaid where r3 measured them (freeze=hid s1 25.2 > 2/η while clean, committed-reference).
