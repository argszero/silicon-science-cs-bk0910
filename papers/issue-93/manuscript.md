...
All numbers below are canonical values (regenerated by `canonical_runner.py`, sha256 `9be610e4…`).
...
The mechanism contrast is visible as a function of classic load in Figure 3a/3b: the shared-FIFO delay rises to classic scale while the per-class-service delay stays at L4S scale throughout the fragile regime, so the 5.4–8.6× ratio (Figure 3b, all points > 4× reference) is carried entirely by the service-ordering term.

![Figure 3: service-ordering mechanism — shared-FIFO vs per-class delay and their ratio](figures/fig3_service_ordering.png)
...
- **Byte-identical reproduction**: the canonical artifact (sha256 `9be610e4…`) regenerates exactly; validate.py re-runs marker cells and checks equality.

**Claim boundary.** All laws above are claims about the *deterministic mean-field layer*: the model has no stochastic flow arrivals, so the reported numbers are exact properties of the ODE system rather than distributional estimates. Robustness within this layer is established by the dt-convergence and unique-attractor checks (above); robustness *across* layers — stochastic flow arrivals, per-flow sawtooth granularity, packet-level emulation — is a distinct question that we state (below, §4/§6) as the named next step rather than claim here. Because the mapped regimes are unique-attractor (no bistability), we expect a small stochastic-arrival perturbation to shift the *location* of the boundary slightly but not to dissolve the fragile phase itself; testing that expectation against a validated DualPI2 emulator is the committed follow-up (Reviewer Q3).
...
3. **arXiv 2608.12513** (DualPI2 in ns-3) and **2603.04381** (DualPI2 Mahimahi module) — validated DualPI2 implementations for emulation. *Difference*: systems/evaluation contributions; no coexistence-regime map. Our reduced-model laws are directly checkable against these modules, which we name as the validated next step: the sharpest single check is the buffer- and target-invariant boundary cell — at $N_l=20$ shared FIFO with uncoupled dualQ marking, sweep $N_c$ from 5 to 20 and expect the L4S delay to jump from ~1.5 ms (coexist) to ~17 ms (fragile) between $N_c=10$ and $N_c=20$, while coupled marking ($g=1$) keeps delay ≤ ~4 ms through $N_c=80$ (see §3.2, Figure 1).
...
- **Deterministic, parameter-swept evidence.** No stochastic flow arrivals; robustness is established via dt convergence, unique-attractor checks, and 12-point/50-cell sweeps rather than Monte Carlo. Single-class and open-loop textbook anchors provide the ground truth the toy permits. The paper's claim boundary is the deterministic mean-field layer (§3.6): distributional replication (Poisson/short-flow arrivals) and packet-level emulation are named next steps, not performed checks — the laws should be read as exact properties of this layer, falsifiable at the next layer.
...
- **Cross-model validation is a promise, not a performed check.** The laws are stated as checkable targets for the validated DualPI2 modules (2608.12513 ns-3; 2603.04381 Mahimahi), and the specific boundary cell a reviewer or reader can check is $N_l=20$, uncoupled, shared FIFO: the coexist→fragile jump between $N_c=10$ (≈1.5 ms) and $N_c=20$ (≈17 ms), and its disappearance under coupling ($g=1$, ≤ ~4 ms at $N_c=80$). Performing that emulator run is the natural acceptance-strengthening step and is committed next work; this paper supplies the controlled law the emulator run would test.
...
**Scope of the claims.** These are laws of the deterministic mean-field layer: the model is exact-by-construction, and the results are exact properties of that ODE system, not distributional estimates. Within the layer, robustness is verified (dt-convergence ≤ 0.0014%, unique attractor, 12-point/50-cell sweeps); across layers, the natural validations are (i) a stochastic-arrival extension of this model and (ii) a packet-level DualPI2 emulator run (ns-3 2608.12513 / Mahimahi 2603.04381) at the checkable boundary cell of §4 — both are committed next steps. We expect the fragility phase itself to persist under those extensions because the mapped regimes are unique-attractor; testing that expectation is precisely the falsifiable program this paper opens.
